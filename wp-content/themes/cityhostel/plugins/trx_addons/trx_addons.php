...
if (!function_exists('cityhostel_trx_addons_theme_setup1')) {
	add_action( 'after_setup_theme', 'cityhostel_trx_addons_theme_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'cityhostel_trx_addons_theme_setup1', 8 );
	function cityhostel_trx_addons_theme_setup1() {
		if (cityhostel_exists_trx_addons()) {
			add_filter( 'cityhostel_filter_list_posts_types',	'cityhostel_trx_addons_list_post_types');
			add_filter( 'cityhostel_filter_list_header_styles','cityhostel_trx_addons_list_header_styles');
			add_filter( 'cityhostel_filter_list_footer_styles','cityhostel_trx_addons_list_footer_styles');
		}
	}
}
...
if (!function_exists('cityhostel_trx_addons_theme_setup9')) {
	add_action( 'after_setup_theme', 'cityhostel_trx_addons_theme_setup9', 9 );
	function cityhostel_trx_addons_theme_setup9() {
		if (cityhostel_exists_trx_addons()) {
			add_filter( 'trx_addons_sc_atts',							'cityhostel_trx_addons_sc_atts', 10, 2);
			add_filter( 'trx_addons_sc_output',							'cityhostel_trx_addons_sc_output', 10, 4);
			add_filter( 'trx_addons_filter_featured_image',				'cityhostel_trx_addons_featured_image', 10, 2);
			add_filter( 'trx_addons_filter_no_image',					'cityhostel_trx_addons_no_image' );
			add_filter( 'trx_addons_filter_get_list_icons',				'cityhostel_trx_addons_get_list_icons', 10, 2 );
			add_filter( 'trx_addons_filter_slider_title',				'cityhostel_trx_addons_slider_title', 10, 2 );
			add_filter( 'trx_addons_filter_meta_box_fields',			'cityhostel_trx_addons_meta_box_fields', 10, 2);
			add_action( 'wp_enqueue_scripts', 							'cityhostel_trx_addons_frontend_scripts', 1100 );
			add_filter( 'cityhostel_filter_query_sort_order',	 			'cityhostel_trx_addons_query_sort_order', 10, 3);
			add_filter( 'cityhostel_filter_merge_scripts',					'cityhostel_trx_addons_merge_scripts');
			add_filter( 'cityhostel_filter_get_css',						'cityhostel_trx_addons_get_css', 10, 4);
			add_filter( 'cityhostel_filter_prepare_css',					'cityhostel_trx_addons_prepare_css', 10, 2);
			add_filter( 'cityhostel_filter_prepare_js',					'cityhostel_trx_addons_prepare_js', 10, 2);
			add_filter( 'cityhostel_filter_get_post_categories',		 	'cityhostel_trx_addons_get_post_categories');
			add_filter( 'cityhostel_filter_get_post_date',		 			'cityhostel_trx_addons_get_post_date');
			add_filter( 'trx_addons_filter_get_post_date',		 		'cityhostel_trx_addons_get_post_date_wrap');
			add_filter( 'cityhostel_filter_post_type_taxonomy',			'cityhostel_trx_addons_post_type_taxonomy', 10, 2 );
			if (is_admin()) {
				add_filter( 'cityhostel_filter_allow_override', 			'cityhostel_trx_addons_allow_override', 10, 2);
				add_filter( 'cityhostel_filter_allow_theme_icons', 		'cityhostel_trx_addons_allow_theme_icons', 10, 2);
				add_filter( 'trx_addons_sc_map',						'cityhostel_trx_addons_sc_map', 10, 2);
			} else {
				add_filter( 'trx_addons_filter_theme_logo',				'cityhostel_trx_addons_theme_logo');
				add_filter( 'trx_addons_filter_post_meta',				'cityhostel_trx_addons_post_meta', 10, 2);
				add_filter( 'cityhostel_filter_get_mobile_menu',			'cityhostel_trx_addons_get_mobile_menu');
				add_filter( 'cityhostel_filter_detect_blog_mode',			'cityhostel_trx_addons_detect_blog_mode' );
				add_filter( 'cityhostel_filter_get_blog_title', 			'cityhostel_trx_addons_get_blog_title');
				add_action( 'cityhostel_action_login',						'cityhostel_trx_addons_action_login', 10, 2);
				add_action( 'cityhostel_action_search',					'cityhostel_trx_addons_action_search', 10, 3);
				add_action( 'cityhostel_action_breadcrumbs',				'cityhostel_trx_addons_action_breadcrumbs');
				add_action( 'cityhostel_action_show_layout',				'cityhostel_trx_addons_action_show_layout', 10, 1);
			}
		}
		
		// Add this filter any time: if plugin exists - load plugin's styles, if not exists - load layouts.css instead plugin's styles
		add_filter( 'cityhostel_filter_merge_styles',						'cityhostel_trx_addons_merge_styles');
		
		if (is_admin()) {
			add_filter( 'cityhostel_filter_tgmpa_required_plugins',		'cityhostel_trx_addons_tgmpa_required_plugins' );
			add_action( 'admin_enqueue_scripts', 						'cityhostel_trx_addons_editor_load_scripts_admin');
		}
	}
}

// Filter to add in the required plugins list
if ( !function_exists( 'cityhostel_trx_addons_tgmpa_required_plugins' ) ) {
	//Handler of the add_filter('cityhostel_filter_tgmpa_required_plugins',	'cityhostel_trx_addons_tgmpa_required_plugins');
	function cityhostel_trx_addons_tgmpa_required_plugins($list=array()) {
		if (in_array('trx_addons', cityhostel_storage_get('required_plugins'))) {
			$path = cityhostel_get_file_dir('plugins/trx_addons/trx_addons.zip');
			$list[] = array(
					'name' 		=> esc_html__('ThemeREX Addons', 'cityhostel'),
					'slug' 		=> 'trx_addons',
					'version'	=> '1.6.17',
...
if ( !function_exists( 'cityhostel_exists_trx_addons' ) ) {
	function cityhostel_exists_trx_addons() {
		return defined('TRX_ADDONS_VERSION');
	}
}

// Return true if team is supported
if ( !function_exists( 'cityhostel_exists_team' ) ) {
	function cityhostel_exists_team() {
		return defined('TRX_ADDONS_CPT_TEAM_PT');
	}
}

// Return true if services is supported
if ( !function_exists( 'cityhostel_exists_services' ) ) {
	function cityhostel_exists_services() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT');
	}
}

// Return true if portfolio is supported
if ( !function_exists( 'cityhostel_exists_portfolio' ) ) {
	function cityhostel_exists_portfolio() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT');
	}
}

// Return true if courses is supported
if ( !function_exists( 'cityhostel_exists_courses' ) ) {
	function cityhostel_exists_courses() {
		return defined('TRX_ADDONS_CPT_COURSES_PT');
	}
}

// Return true if layouts is supported
if ( !function_exists( 'cityhostel_exists_layouts' ) ) {
	function cityhostel_exists_layouts() {
		return defined('TRX_ADDONS_CPT_LAYOUTS_PT');
	}
}

// Return true if dishes is supported
if ( !function_exists( 'cityhostel_exists_dishes' ) ) {
	function cityhostel_exists_dishes() {
		return defined('TRX_ADDONS_CPT_DISHES_PT');
	}
}

// Return true if it's team page
if ( !function_exists( 'cityhostel_is_team_page' ) ) {
	function cityhostel_is_team_page() {
		return defined('TRX_ADDONS_CPT_TEAM_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'cityhostel_is_services_page' ) ) {
	function cityhostel_is_services_page() {
		return defined('TRX_ADDONS_CPT_SERVICES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'cityhostel_is_portfolio_page' ) ) {
	function cityhostel_is_portfolio_page() {
		return defined('TRX_ADDONS_CPT_PORTFOLIO_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'cityhostel_is_courses_page' ) ) {
	function cityhostel_is_courses_page() {
		return defined('TRX_ADDONS_CPT_COURSES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'cityhostel_is_dishes_page' ) ) {
	function cityhostel_is_dishes_page() {
		return defined('TRX_ADDONS_CPT_DISHES_PT') 
					&& !is_search()
					&& (
...
if ( !function_exists( 'cityhostel_trx_addons_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'cityhostel_trx_addons_frontend_scripts', 1100 );
	function cityhostel_trx_addons_frontend_scripts() {
		if (cityhostel_exists_trx_addons()) {
			if (cityhostel_is_on(cityhostel_get_theme_option('debug_mode')) && cityhostel_get_file_dir('plugins/trx_addons/trx_addons.css')!='') {
				wp_enqueue_style( 'cityhostel-trx_addons',  cityhostel_get_file_url('plugins/trx_addons/trx_addons.css'), array(), null );
				wp_enqueue_style( 'cityhostel-trx_addons_editor',  cityhostel_get_file_url('plugins/trx_addons/trx_addons.editor.css'), array(), null );
			}
			if (cityhostel_is_on(cityhostel_get_theme_option('debug_mode')) && cityhostel_get_file_dir('plugins/trx_addons/trx_addons.js')!='')
				wp_enqueue_script( 'cityhostel-trx_addons', cityhostel_get_file_url('plugins/trx_addons/trx_addons.js'), array('jquery'), null, true );
		} else {
			// Load custom layouts from the theme if plugin not exists
			if ( cityhostel_is_on(cityhostel_get_theme_option('debug_mode')) ) {
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/layouts.css') );
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/logo.css') );
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/menu.css') );
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/search.css') );
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/title.css') );
				wp_enqueue_style( 'cityhostel-layouts', cityhostel_get_file_url('plugins/trx_addons/layouts/featured.css') );
			}
		}
	}
}
...
// Merge custom styles
if ( !function_exists( 'cityhostel_trx_addons_merge_styles' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_merge_styles', 'cityhostel_trx_addons_merge_styles');
	function cityhostel_trx_addons_merge_styles($list) {
...
if ( !function_exists( 'cityhostel_trx_addons_merge_scripts' ) ) {
	//Handler of the add_filter('cityhostel_filter_merge_scripts', 'cityhostel_trx_addons_merge_scripts');
	function cityhostel_trx_addons_merge_scripts($list) {
...
if ( !function_exists( 'cityhostel_trx_addons_detect_blog_mode' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_detect_blog_mode', 'cityhostel_trx_addons_detect_blog_mode' );
	function cityhostel_trx_addons_detect_blog_mode($mode='') {
		if ( cityhostel_is_team_page() )
			$mode = 'team';
		else if ( cityhostel_is_services_page() )
			$mode = 'services';
		else if ( cityhostel_is_portfolio_page() )
			$mode = 'portfolio';
		else if ( cityhostel_is_courses_page() )
			$mode = 'courses';
		else if ( cityhostel_is_dishes_page() )
			$mode = 'dishes';
		return $mode;
	}
}
...
if ( !function_exists( 'cityhostel_trx_addons_list_post_types' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_list_posts_types', 'cityhostel_trx_addons_list_post_types');
	function cityhostel_trx_addons_list_post_types($list=array()) {
...
if ( !function_exists( 'cityhostel_trx_addons_post_type_taxonomy' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_post_type_taxonomy',	'cityhostel_trx_addons_post_type_taxonomy', 10, 2 );
	function cityhostel_trx_addons_post_type_taxonomy($tax='', $post_type='') {
...
if ( !function_exists( 'cityhostel_trx_addons_get_post_categories' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_get_post_categories', 		'cityhostel_trx_addons_get_post_categories');
	function cityhostel_trx_addons_get_post_categories($cats='') {

		if ( defined('TRX_ADDONS_CPT_TEAM_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_TEAM_PT) {
				$cats = cityhostel_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_TEAM_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_SERVICES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_SERVICES_PT) {
				$cats = cityhostel_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_SERVICES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_PORTFOLIO_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_PORTFOLIO_PT) {
				$cats = cityhostel_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_PORTFOLIO_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				$cats = cityhostel_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_COURSES_TAXONOMY);
			}
		}
		if ( defined('TRX_ADDONS_CPT_DISHES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_DISHES_PT) {
				$cats = cityhostel_get_post_terms(', ', get_the_ID(), TRX_ADDONS_CPT_DISHES_TAXONOMY);
			}
		}
		return $cats;
	}
...
if ( !function_exists( 'cityhostel_trx_addons_list_header_styles' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_list_header_styles', 'cityhostel_trx_addons_list_header_styles');
	function cityhostel_trx_addons_list_header_styles($list=array()) {
		if (cityhostel_exists_layouts()) {
			$layouts = cityhostel_get_list_posts(false, array(
...
if ( !function_exists( 'cityhostel_trx_addons_list_footer_styles' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_list_footer_styles', 'cityhostel_trx_addons_list_footer_styles');
	function cityhostel_trx_addons_list_footer_styles($list=array()) {
		if (cityhostel_exists_layouts()) {
			$layouts = cityhostel_get_list_posts(false, array(
							'post_type' => TRX_ADDONS_CPT_LAYOUTS_PT,
							'meta_key' => 'trx_addons_layout_type',
...
if ( !function_exists( 'cityhostel_trx_addons_get_post_date_wrap' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_post_date', 'cityhostel_trx_addons_get_post_date_wrap');
	function cityhostel_trx_addons_get_post_date_wrap($dt='') {
		return apply_filters('cityhostel_filter_get_post_date', $dt);
	}
}

// Show date of the courses
if ( !function_exists( 'cityhostel_trx_addons_get_post_date' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_get_post_date', 'cityhostel_trx_addons_get_post_date');
	function cityhostel_trx_addons_get_post_date($dt='') {
		...
		if ( defined('TRX_ADDONS_CPT_COURSES_PT') ) {
			if (get_post_type()==TRX_ADDONS_CPT_COURSES_PT) {
				...
				$dt = sprintf($dt < date('Y-m-d') 
						? esc_html__('Started on %s', 'cityhostel') 
						: esc_html__('Starting %s', 'cityhostel'), 
...
if (!function_exists('cityhostel_trx_addons_allow_override')) {
	//Handler of the add_filter( 'cityhostel_filter_allow_override', 'cityhostel_trx_addons_allow_override', 10, 2);
	function cityhostel_trx_addons_allow_override($allow, $post_type) {
		return $allow
...
if (!function_exists('cityhostel_trx_addons_allow_theme_icons')) {
	//Handler of the add_filter( 'cityhostel_filter_allow_theme_icons', 'cityhostel_trx_addons_allow_theme_icons', 10, 2);
	function cityhostel_trx_addons_allow_theme_icons($allow, $post_type) {
		return $allow
...
if (!function_exists('cityhostel_trx_addons_meta_box_fields')) {
	//Handler of the add_filter( 'trx_addons_filter_meta_box_fields', 'cityhostel_trx_addons_meta_box_fields', 10, 2);
	function cityhostel_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'cityhostel'),
				"desc" => wp_kses_data( __("Team member's email", 'cityhostel') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
...
if ( !function_exists( 'cityhostel_trx_addons_editor_load_scripts_admin' ) ) {
	//Handler of the add_action("admin_enqueue_scripts", 'cityhostel_trx_addons_editor_load_scripts_admin');
	function cityhostel_trx_addons_editor_load_scripts_admin() {
		// Add styles in the WP text editor
		add_editor_style( array(
							cityhostel_get_file_url('plugins/trx_addons/trx_addons.editor.css')
							)
						 );	
	}
}
...
if ( !function_exists( 'cityhostel_trx_addons_sc_atts' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_atts', 'cityhostel_trx_addons_sc_atts', 10, 2);
	function cityhostel_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'cityhostel_trx_addons_sc_map' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_map', 'cityhostel_trx_addons_sc_map', 10, 2);
	function cityhostel_trx_addons_sc_map($params, $sc) {
...
if ( !function_exists( 'cityhostel_trx_addons_sc_output' ) ) {
	//Handler of the add_filter( 'trx_addons_sc_output', 'cityhostel_trx_addons_sc_output', 10, 4);
	function cityhostel_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_layouts_featured')))
			cityhostel_sc_layouts_showed('featured', is_singular() && in_array(get_post_type(), array('post', 'page')) && has_post_thumbnail());
			...
			cityhostel_sc_layouts_showed('cart', true);
		else if (in_array($sc, array('trx_sc_layouts_logo')))
			cityhostel_sc_layouts_showed('logo', true);
		else if (in_array($sc, array('trx_sc_layouts_menu')))
			cityhostel_sc_layouts_showed('menu', true);
		else if (in_array($sc, array('trx_sc_layouts_search')))
			cityhostel_sc_layouts_showed('search', true);
...
if ( !function_exists( 'cityhostel_trx_addons_slider_title' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_slider_title',	'cityhostel_trx_addons_slider_title', 10, 2 );
	function cityhostel_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if (!function_exists('cityhostel_sc_layouts_showed')) {
	function cityhostel_sc_layouts_showed($name, $val=null) {
		if ($val!==null)
			return cityhostel_storage_set_array('sc_layouts_components', $name, $val);
		else
			return cityhostel_storage_get_array('sc_layouts_components', $name) != '';
	}
}

// Return image size multiplier
if (!function_exists('cityhostel_get_retina_multiplier')) {
	function cityhostel_get_retina_multiplier($force_retina=0) {
		static $mult = 0;
...
if (!function_exists('cityhostel_build_slider_layout')) {
	function cityhostel_build_slider_layout($args) {
...
if ( !function_exists( 'cityhostel_trx_addons_featured_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_featured_image', 'cityhostel_trx_addons_featured_image', 10, 2);
	function cityhostel_trx_addons_featured_image($processed=false, $args=array()) {
		$args['show_no_image'] = true;
		$args['singular'] = false;
		$args['hover'] = cityhostel_get_theme_option('image_hover');
		cityhostel_show_post_featured($args);
		return true;
	}
}

// Return theme specific 'no-image' picture
if ( !function_exists( 'cityhostel_trx_addons_no_image' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_no_image', 'cityhostel_trx_addons_no_image');
	function cityhostel_trx_addons_no_image($no_image='') {
		return cityhostel_get_no_image($no_image);
	}
}

// Return theme-specific icons
if ( !function_exists( 'cityhostel_trx_addons_get_list_icons' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_get_list_icons', 'cityhostel_trx_addons_get_list_icons', 10, 2 );
	function cityhostel_trx_addons_get_list_icons($list, $prepend_inherit) {
		return cityhostel_get_list_icons($prepend_inherit);
	}
}

// Return links to the social profiles
if (!function_exists('cityhostel_get_socials_links')) {
	function cityhostel_get_socials_links($style='icons') {
...
if (!function_exists('cityhostel_get_share_links')) {
	function cityhostel_get_share_links($args=array()) {
...
if (!function_exists('cityhostel_show_share_links')) {
	function cityhostel_show_share_links($args=array()) {
...
if (!function_exists('cityhostel_get_category_image')) {
	function cityhostel_get_category_image($term_id=0) {
...
if (!function_exists('cityhostel_get_category_icon')) {
	function cityhostel_get_category_icon($term_id=0) {
...
if (!function_exists('cityhostel_get_post_counters')) {
	function cityhostel_get_post_counters($counters='views') {
...
if (!function_exists('cityhostel_get_list_animations_in')) {
	function cityhostel_get_list_animations_in() {
...
if (!function_exists('cityhostel_get_animation_classes')) {
	function cityhostel_get_animation_classes($animation, $speed='normal', $loop='none') {
...
if (!function_exists('cityhostel_get_comment_counters')) {
	function cityhostel_get_comment_counters($counters = 'likes') {
...
if (!function_exists('cityhostel_show_comment_counters')) {
	function cityhostel_show_comment_counters($counters = 'likes') {
...
if (!function_exists('cityhostel_trx_addons_query_sort_order')) {
	//Handler of the add_filter('cityhostel_filter_query_sort_order', 'cityhostel_trx_addons_query_sort_order', 10, 3);
	function cityhostel_trx_addons_query_sort_order($q=array(), $orderby='date', $order='desc') {
		if ($orderby == 'views') {
...
if ( !function_exists( 'cityhostel_trx_addons_theme_logo' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_theme_logo', 'cityhostel_trx_addons_theme_logo');
	function cityhostel_trx_addons_theme_logo($logo) {
		return cityhostel_get_logo_image();
	}
}

// Return theme-specific post meta to the plugin
if ( !function_exists( 'cityhostel_trx_addons_post_meta' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_post_meta',	'cityhostel_trx_addons_post_meta', 10, 2);
	function cityhostel_trx_addons_post_meta($meta, $args=array()) {
		return cityhostel_show_post_meta($args);
	}
}
	
// Redirect action 'get_mobile_menu' to the plugin
// Return stored items as mobile menu
if ( !function_exists( 'cityhostel_trx_addons_get_mobile_menu' ) ) {
	//Handler of the add_filter("cityhostel_filter_get_mobile_menu", 'cityhostel_trx_addons_get_mobile_menu');
	function cityhostel_trx_addons_get_mobile_menu($menu) {
		return apply_filters('trx_addons_filter_get_mobile_menu', $menu);
	}
}

// Redirect action 'login' to the plugin
if (!function_exists('cityhostel_trx_addons_action_login')) {
	//Handler of the add_action( 'cityhostel_action_login',		'cityhostel_trx_addons_action_login', 10, 2);
	function cityhostel_trx_addons_action_login($link_text='', $link_title='') {
		do_action( 'trx_addons_action_login', $link_text, $link_title );
	}
}

// Redirect action 'search' to the plugin
if (!function_exists('cityhostel_trx_addons_action_search')) {
	//Handler of the add_action( 'cityhostel_action_search', 'cityhostel_trx_addons_action_search', 10, 3);
	function cityhostel_trx_addons_action_search($style, $class, $ajax) {
		do_action( 'trx_addons_action_search', $style, $class, $ajax );
	}
}

// Redirect action 'breadcrumbs' to the plugin
if (!function_exists('cityhostel_trx_addons_action_breadcrumbs')) {
	//Handler of the add_action( 'cityhostel_action_breadcrumbs',	'cityhostel_trx_addons_action_breadcrumbs');
	function cityhostel_trx_addons_action_breadcrumbs() {
		do_action( 'trx_addons_action_breadcrumbs' );
	}
}

// Redirect action 'show_layout' to the plugin
if (!function_exists('cityhostel_trx_addons_action_show_layout')) {
	//Handler of the add_action( 'cityhostel_action_show_layout', 'cityhostel_trx_addons_action_show_layout', 10, 1);
	function cityhostel_trx_addons_action_show_layout($layout_id='') {
		do_action( 'trx_addons_action_show_layout', $layout_id );
	}
}

// Redirect filter 'get_blog_title' to the plugin
if ( !function_exists( 'cityhostel_trx_addons_get_blog_title' ) ) {
	//Handler of the add_filter( 'cityhostel_filter_get_blog_title', 'cityhostel_trx_addons_get_blog_title');
	function cityhostel_trx_addons_get_blog_title($title='') {
		return apply_filters('trx_addons_filter_get_blog_title', $title);
	}
}

// Redirect filter 'prepare_css' to the plugin
if (!function_exists('cityhostel_trx_addons_prepare_css')) {
	//Handler of the add_filter( 'cityhostel_filter_prepare_css',	'cityhostel_trx_addons_prepare_css', 10, 2);
	function cityhostel_trx_addons_prepare_css($css='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_css', $css, $remove_spaces );
	}
}

// Redirect filter 'prepare_js' to the plugin
if (!function_exists('cityhostel_trx_addons_prepare_js')) {
	//Handler of the add_filter( 'cityhostel_filter_prepare_js',	'cityhostel_trx_addons_prepare_js', 10, 2);
	function cityhostel_trx_addons_prepare_js($js='', $remove_spaces=true) {
		return apply_filters( 'trx_addons_filter_prepare_js', $js, $remove_spaces );
	}
}

// Return text for the "I agree ..." checkbox
if ( ! function_exists( 'cityhostel_trx_addons_privacy_text' ) ) {
    add_filter( 'trx_addons_filter_privacy_text', 'cityhostel_trx_addons_privacy_text' );
    function cityhostel_trx_addons_privacy_text( $text='' ) {
        return cityhostel_get_privacy_text();
    }
}

// Add theme-specific options to the post's options
if (!function_exists('cityhostel_trx_addons_override_options')) {
    add_filter( 'trx_addons_filter_override_options', 'cityhostel_trx_addons_override_options');
    function cityhostel_trx_addons_override_options($options=array()) {
        return apply_filters('cityhostel_filter_override_options', $options);
    }
}
...
if (!function_exists('cityhostel_trx_addons_get_css')) {
	//Handler of the add_filter('cityhostel_filter_get_css', 'cityhostel_trx_addons_get_css', 10, 4);
	function cityhostel_trx_addons_get_css($css, $colors, $fonts, $scheme='') {
...
.slider_swiper.slider_pagination.slider_multi.swiper-container-horizontal > .swiper-pagination .swiper-pagination-bullet {
    background-color: {$colors['alter_text']};
}

.slider_swiper .slider_pagination_wrap .swiper-pagination-bullet.swiper-pagination-bullet-active:after,
.slider_swiper_outer .slider_pagination_wrap .swiper-pagination-bullet.swiper-pagination-bullet-active:after,
.slider_swiper .slider_pagination_wrap .swiper-pagination-bullet:hover:after,
.slider_swiper_outer .slider_pagination_wrap .swiper-pagination-bullet:hover:after {
	border-color: {$colors['alter_link']};
}
...
.sc_item_button a.sc_button_bordered:not(.sc_button_bg_image):hover {
	color:{$colors['alter_link']} !important;
	border-color: {$colors['alter_link']};
	background-color:{$colors['bg_color_0']};
}
...
.sc_skills .sc_skills_column:nth-child(2n) .sc_skills_total,
.sc_skills .sc_skills_column:nth-child(2n) .sc_skills_item_title,
.sc_skills .sc_skills_column:nth-child(2n) .sc_skills_legend_title,
.sc_skills .sc_skills_column:nth-child(2n) .sc_skills_legend_value {
	color:{$colors['text']};
}
