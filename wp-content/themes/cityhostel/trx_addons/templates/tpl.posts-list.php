...
if ( $trx_addons_show_counters && ($trx_addons_counters_list = $trx_addons_args['counters']) !='' ) {
	$trx_addons_post_counters_output = '<span class="post_info_item post_info_counters">'
								. trx_addons_get_post_counters('comments')
							. '</span>';
}
...
if ($trx_addons_show_image) {
	$trx_addons_post_thumb = get_the_post_thumbnail($trx_addons_post_id, trx_addons_get_thumb_size('posts'), array(
		'alt' => get_the_title()
	));
...
$trx_addons_output .= '<div class="post_content">'
			...
			. '<div class="post_title">' . ($trx_addons_post_link ? '<a href="' . esc_url($trx_addons_post_link) . '">' : '') . ($trx_addons_post_title) . ($trx_addons_post_link ? '</a>' : '') . '</div>'
			. '<div class="post_info">'
