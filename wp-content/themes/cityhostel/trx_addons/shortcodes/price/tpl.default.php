...
?>><?php
	...
	if (!empty($args['image'])) {
		if (!empty($args['link'])) {
            ...
            if (!empty($args['price_discount'])) {
                ?><div class="sc_price_price_discount"><?php echo wp_kses_post(trx_addons_parse_codes($args['price_discount'])); ?></div><?php
            }

            if ( ($content = do_shortcode($content))!='' ) {
                ?><div class="sc_price_details"><?php echo wp_kses_post($content); ?></div><?php
            }
		$args['image'] = trx_addons_get_attachment_url($args['image'], trx_addons_get_thumb_size('price'));
		$attr = trx_addons_getimagesize($args['image']);
		?><img src="<?php echo esc_url($args['image']); ?>" alt="<?php echo esc_html(basename($args['image'])); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>><?php
		if (!empty($args['link'])) {
			?></a><?php
		} else {
			?></div><?php
