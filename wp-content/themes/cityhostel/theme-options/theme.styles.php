...
if (!function_exists('cityhostel_customizer_add_theme_colors')) {
	function cityhostel_customizer_add_theme_colors($colors) {
		if (substr($colors['text'], 0, 1) == '#') {
			$colors['bg_color_0']  = cityhostel_hex2rgba( $colors['bg_color'], 0 );
			$colors['bg_color_02']  = cityhostel_hex2rgba( $colors['bg_color'], 0.2 );
            $colors['bg_color_04']  = cityhostel_hex2rgba( $colors['bg_color'], 0.4 );
			$colors['bg_color_07']  = cityhostel_hex2rgba( $colors['bg_color'], 0.7 );
			$colors['bg_color_08']  = cityhostel_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['bg_color_09']  = cityhostel_hex2rgba( $colors['bg_color'], 0.9 );
			$colors['alter_bg_color_07']  = cityhostel_hex2rgba( $colors['alter_bg_color'], 0.7 );
			$colors['alter_bg_color_04']  = cityhostel_hex2rgba( $colors['alter_bg_color'], 0.4 );
			$colors['alter_bg_color_02']  = cityhostel_hex2rgba( $colors['alter_bg_color'], 0.2 );
			$colors['alter_bd_color_02']  = cityhostel_hex2rgba( $colors['alter_bd_color'], 0.2 );
            $colors['text_dark_01']  = cityhostel_hex2rgba( $colors['text_dark'], 0.1 );
			$colors['text_dark_07']  = cityhostel_hex2rgba( $colors['text_dark'], 0.7 );
            $colors['text_dark_08']  = cityhostel_hex2rgba( $colors['text_dark'], 0.8 );
            $colors['alter_bg_color_09']  = cityhostel_hex2rgba( $colors['alter_bg_color'], 0.9 );
			$colors['text_link_02']  = cityhostel_hex2rgba( $colors['text_link'], 0.2 );
			$colors['text_link_07']  = cityhostel_hex2rgba( $colors['text_link'], 0.7 );
            $colors['text_02']  = cityhostel_hex2rgba( $colors['text'], 0.2 );
			$colors['text_link_blend'] = cityhostel_hsb2hex(cityhostel_hex2hsb( $colors['text_link'], 2, -5, 5 ));
			$colors['alter_link_blend'] = cityhostel_hsb2hex(cityhostel_hex2hsb( $colors['alter_link'], 2, -5, 5 ));
		} else {
...
if (!function_exists('cityhostel_customizer_add_theme_fonts')) {
	function cityhostel_customizer_add_theme_fonts($fonts) {
		$rez = array();	
		foreach ($fonts as $tag => $font) {
			//$rez[$tag] = $font;
			if (substr($font['font-family'], 0, 2) != '{{') {
				$rez[$tag.'_font-family'] 		= !empty($font['font-family']) && !cityhostel_is_inherit($font['font-family'])
														? 'font-family:' . trim($font['font-family']) . ';' 
														: '';
				$rez[$tag.'_font-size'] 		= !empty($font['font-size']) && !cityhostel_is_inherit($font['font-size'])
														? 'font-size:' . cityhostel_prepare_css_value($font['font-size']) . ";"
														: '';
				$rez[$tag.'_line-height'] 		= !empty($font['line-height']) && !cityhostel_is_inherit($font['line-height'])
														? 'line-height:' . trim($font['line-height']) . ";"
														: '';
				$rez[$tag.'_font-weight'] 		= !empty($font['font-weight']) && !cityhostel_is_inherit($font['font-weight'])
														? 'font-weight:' . trim($font['font-weight']) . ";"
														: '';
				$rez[$tag.'_font-style'] 		= !empty($font['font-style']) && !cityhostel_is_inherit($font['font-style'])
														? 'font-style:' . trim($font['font-style']) . ";"
														: '';
				$rez[$tag.'_text-decoration'] 	= !empty($font['text-decoration']) && !cityhostel_is_inherit($font['text-decoration'])
														? 'text-decoration:' . trim($font['text-decoration']) . ";"
														: '';
				$rez[$tag.'_text-transform'] 	= !empty($font['text-transform']) && !cityhostel_is_inherit($font['text-transform'])
														? 'text-transform:' . trim($font['text-transform']) . ";"
														: '';
				$rez[$tag.'_letter-spacing'] 	= !empty($font['letter-spacing']) && !cityhostel_is_inherit($font['letter-spacing'])
														? 'letter-spacing:' . trim($font['letter-spacing']) . ";"
														: '';
				$rez[$tag.'_margin-top'] 		= !empty($font['margin-top']) && !cityhostel_is_inherit($font['margin-top'])
														? 'margin-top:' . cityhostel_prepare_css_value($font['margin-top']) . ";"
														: '';
				$rez[$tag.'_margin-bottom'] 	= !empty($font['margin-bottom']) && !cityhostel_is_inherit($font['margin-bottom'])
														? 'margin-bottom:' . cityhostel_prepare_css_value($font['margin-bottom']) . ";"
														: '';
			} else {
...
if (!function_exists('cityhostel_customizer_get_css')) {

	function cityhostel_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = cityhostel_customizer_add_theme_fonts($fonts);
...
.more-link,
.cityhostel_tabs .cityhostel_tabs_titles li a {
...
.trx_addons_dropcap.trx_addons_dropcap_style_3,
...
CSS;
			$rez = apply_filters('cityhostel_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(cityhostel_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = cityhostel_get_scheme_colors($scheme);
	
					// Make theme-specific colors and tints
					$colors = cityhostel_customizer_add_theme_colors($colors);
...
.wpcf7-form-control-wrap.date input::-webkit-input-placeholder { color: {$colors['text']}; }
.wpcf7-form-control-wrap.date input::-moz-placeholder          { color: {$colors['text']}; }
.wpcf7-form-control-wrap.date input:-ms-input-placeholder      { color: {$colors['text']}; }
...
.header_position_over.top_panel_fixed .sc_layouts_row_fixed {
    background-color: {$colors['alter_dark']};
}

.header_position_over .sc_layouts_row_fixed .sc_layouts_menu_nav a {
	color: {$colors['inverse_text']};
}
...
.footer_wrap .widget .post_item .post_info .post_counters_item:before,
.sc_blogger.sc_blogger_default .post_meta .post_meta_item.post_counters_item:before {
    color: {$colors['inverse_text']};
	background-color: {$colors['text_link']};
}
.textwidget {
...
.cityhostel_tabs .cityhostel_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.cityhostel_tabs .cityhostel_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
...
.sc_layouts_menu_mobile_button_burger:not(.without_menu) .sc_layouts_item_icon {
    color: {$colors['inverse_text']};
    background-color: {$colors['text_dark']};
}
.sc_layouts_menu_mobile_button_burger:not(.without_menu) .sc_layouts_item_icon:hover {
	color: {$colors['text_dark']};
	background-color: {$colors['alter_link']};
}
.sc_layouts_row_type_compact .sc_layouts_item_details_line1 {
...
.footer_wrap .scheme_dark .post_info .post_info_item a  {
	color: {$colors['inverse_text']};
}
.footer_wrap .scheme_dark .post_info .post_info_item a:hover  {
	color: {$colors['text_link']};
}
...
CSS;
					...
					$rez = apply_filters('cityhostel_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'cityhostel_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>