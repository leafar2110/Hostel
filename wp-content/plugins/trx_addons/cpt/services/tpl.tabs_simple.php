			...
			while ( $query->have_posts() ) { $query->the_post();
				...
				if ( has_post_thumbnail() && (empty($args['featured']) || $args['featured']=='image')) {
					set_query_var('trx_addons_args_featured', array(
						...
						'thumb_size' => apply_filters('trx_addons_filter_services_thumb_size', trx_addons_get_thumb_size($args['columns'] < 3 ? 'medium' : 'small'))
					));
					ob_start();
					...
					$tabs_item .= '<span id="'.esc_attr($args['id'].'_'.trim($meta['icon'])).'"'
										. ' class="sc_services_item_icon '.(empty($svg) ? esc_attr($meta['icon']) : 'sc_icon_type_svg').'">'
										. (!empty($svg) ? trx_addons_get_svg_from_file($svg) : '')
									. '</span>';
