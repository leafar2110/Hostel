...
if ($query->found_posts > 0) {
	...
	?><div class="sc_dishes sc_dishes_<?php 
		...
		if ($args['slider']) {
			?><div class="sc_dishes_slider sc_item_slider slides swiper-wrapper"><?php
		} else if ($args['columns'] > 1) {
			?><div class="sc_dishes_columns sc_item_columns <?php echo esc_attr(trx_addons_get_columns_wrap_class()).($args['type']!='list' ? ' columns_padding_bottom' : ''); ?>"><?php
		} else {
			?><div class="sc_dishes_content sc_item_content"><?php
		}	

		set_query_var('trx_addons_args_sc_dishes', $args);
			
		while ( $query->have_posts() ) { $query->the_post();
			if (($fdir = trx_addons_get_file_dir('cpt/dishes/tpl.'.trx_addons_esc($args['type']).'-item.php')) != '') { include $fdir; }
