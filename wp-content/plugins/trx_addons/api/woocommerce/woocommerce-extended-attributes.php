...
if (!function_exists('trx_addons_woocommerce_attrib_show_custom_fields')) {
	...
	function trx_addons_woocommerce_attrib_show_custom_fields($term) {
		...
		if ($type == 'image') {
			...
			trx_addons_show_layout(trx_addons_options_show_custom_field($field_name.'_button', array('type' => 'mediamanager', 'linked_field_id' => $field_name), null));
			if (empty($term_val)) $term_val = apply_filters('trx_addons_filter_no_thumb', trx_addons_get_file_url('css/images/no-thumb.gif'));
			?><img src="<?php echo esc_url($term_val); ?>" class="trx_addons_thumb_selector_preview <?php echo esc_attr($field_name); ?>_preview"><?php
		} else if ($type == 'color') {
			...
			?><select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'trx_addons' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo $i; ?>][]">
				...
				if ( $all_terms ) {
					foreach ( $all_terms as $term ) {
						echo '<option value="' . esc_attr( $term->slug ) . '" ' . selected( has_term( absint( $term->term_id ), wc_attribute_taxonomy_name($attribute_taxonomy->attribute_name), $thepostid ), true, false ) . '>' . esc_attr( apply_filters( 'woocommerce_product_attribute_term_name', $term->name, $term ) ) . '</option>';
					}
				}
			?></select>
		...
		if ( in_array($type, array('image', 'color', 'button')) ) {
			...
			if ( !empty( $args['options'] ) ) {
				if ( $args['product'] && taxonomy_exists( $args['attribute'] ) ) {
					...
					$terms = wc_get_product_terms( $args['product']->id, $args['attribute'], array( 'fields' => 'all' ) );
			...
			if ($output) {
				...
				$html .= '<div id="' . esc_attr( $args['id'] ? $args['id'] : sanitize_title( $args['attribute'] ) ) . '_attrib_extended" class="' . esc_attr( $args['class'] ? $args['class'] : sanitize_title( $args['attribute'] ) ) . '_attrib_extended trx_addons_attrib_extended" data-attrib="' . esc_attr($args['attribute']) . '">'
							. $output
						. '</div>';
			}
		}
