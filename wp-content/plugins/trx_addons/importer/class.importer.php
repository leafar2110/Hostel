...
class trx_addons_demo_data_importer {
	...
	function admin_scripts() {
		wp_enqueue_style(  'trx_addons-importer',  trx_addons_get_file_url('importer/importer.css'), array(), null );
		wp_enqueue_script( 'trx_addons-importer', trx_addons_get_file_url('importer/importer.js'), array('jquery'), null, true );
	...
	function show_exporter_fields($args=array()) {
		...
		<tr>
			...
			<td><a download="<?php echo esc_attr(!empty($args['download']) ? $args['download'] : $args['slug'].'.txt'); ?>" href="<?php echo esc_url(trx_addons_get_file_url('importer/export/'.trim($args['slug']).'.txt')); ?>"><?php esc_html_e('Download', 'trx_addons'); ?></a></td>
		</tr>
		<?php
	}
	...
	function get_list_pages_from_demo($demo_type) {
		...
		if (!$list || !is_array($list)) {
			...
			set_transient("trx_addons_installer_posts", $list, 60*60);
		}
		return $list;
	}
