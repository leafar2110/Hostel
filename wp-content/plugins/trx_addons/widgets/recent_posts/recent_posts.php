...
class trx_addons_widget_recent_posts extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if (!empty($output)) {
			set_query_var('trx_addons_args_widget_recent_posts', array_merge($args, compact('title', 'output')));
			if (($fdir = trx_addons_get_file_dir('widgets/recent_posts/tpl.default.php')) != '') { include $fdir; }
		}
	}
...
if ( !function_exists( 'trx_addons_sc_widget_recent_posts' ) ) {
	...
	if (trx_addons_exists_visual_composer()) add_shortcode("trx_widget_recent_posts", "trx_addons_sc_widget_recent_posts");
}
...
if (!function_exists('trx_addons_sc_widget_recent_posts_add_in_vc')) {
	...
	if (trx_addons_exists_visual_composer()) add_action('after_setup_theme', 'trx_addons_sc_widget_recent_posts_add_in_vc', 20);
}
?>